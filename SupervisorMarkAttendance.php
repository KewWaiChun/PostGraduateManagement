<?php
	...
	$list = "SELECT DISTINCT Student_ID FROM enrollment WHERE Subject_ID = '$subject' AND Subject_Group = '$group'";
	$GLOBALS['exc'] = $conn->query($list);

	function showList(){
		$var = 0;
	    while ($row = $GLOBALS['exc']->fetch_assoc()){
	      echo "<tr><td>" . $row["Student_ID"] . "</td><td>" . "<input name='pstudent[]' id='pstudent' type='checkbox' value=" . $row["Student_ID"] . "></td><td>" . "<input name='astudent[]' id='astudent' type='checkbox' value=" . $row["Student_ID"] . "></td></tr>";
	    }
  	}
    ...
    <div class="container">
      ...
      <div>
  		...
  		<tr id="horizontalline">
            <th><?php echo $subject . " - " .$group ?></th>
            <th><?php echo $todayDay . " - " .$todayDate ?></th>
        </tr>
      </table>
      <br><br>
      </div>
      <div>
      	<form action="attendance.php" method="post">
          	...
          	<input formaction="SupervisorAttendancePage.php" type="submit" class='btn btn-default' value="Back" />
      	</form>
      </div>
      	<br>
     	<br>
