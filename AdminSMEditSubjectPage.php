<?php
  ...
  $subject="SELECT DISTINCT Subject_ID FROM subject";
  $GLOBALS['getS']=$conn->query($subject);
  
  function showSub(){
    while ($row = $GLOBALS['getS']->fetch_assoc()){
      $sub = $row['Subject_ID'];
...
<html lang="en">
  ...
  <body>
    <div class="container">
      ...
      <div>
        <table>
          <form action="AdminSMEditChooseSubjectGroupPage.php" method="post">
            <?php showSub() ?> 
          </form>
        </table>
      </div>
